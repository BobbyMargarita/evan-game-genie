...
import { getDetail, getSteam, requestDetail, requestSteam, subscribe } from './lib/gameData';
...
function CoverArt({ game, detail, big }) {
  ...
  return (
    ...
    >
      {cover ? (
        <>
          ...
          <div style={{
            ...
            background: 'linear-gradient(to top, rgba(24,20,16,.78), rgba(24,20,16,0))',
          }} />
        </>
      ) : (
        <div style={{
      ...
      <div style={{
        ...
        color: cover ? '#f5ead8' : pal.fg,
        textShadow: cover ? '0 1px 8px rgba(0,0,0,.45)' : 'none',
      }}>
        {game.title}
      </div>
...
function DetailSheet({ game, games, startRect, onClosed, onOpenGame }) {
  const sheetRef = useRef(null);
  const heroRef = useRef(null);
  const bodyRef = useRef(null);
  const detail = getDetail(game.id);
  const ign = ignFor(game.title);
  const appid = detail?.steamAppId;
  const steam = appid ? getSteam(appid) : null;

  useEffect(() => { if (appid) requestSteam(appid); }, [appid]);
  ...
  const showSteam = steam && (steam.series || steam.current != null);
  ...
  return (
    <div ref={sheetRef} className="no-scrollbar" style={{ position: 'absolute', inset: 0, background: 'var(--color-bg)', overflowY: 'auto', overflowX: 'hidden' }}>
      <div ref={heroRef} style={{
        ...
        <CoverArt game={game} detail={detail} big />
      </div>
          ...
          <div style={{ marginTop: 12, background: 'var(--color-surface)', borderRadius: 'var(--radius-lg)', padding: '14px 14px 10px' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 8 }}>
              ...
              <div style={{ font: '700 12px var(--font-body)' }}>
                {steam.series ? `${fmtCount(steam.peak)} peak · 30d` : `${fmtCount(steam.current)} now`}
              </div>
            </div>
            {steam.series && <div style={{ marginTop: 8 }}><Sparkline series={steam.series} /></div>}
            {steam.series && steam.current != null && (
              <div style={{ font: '600 11px var(--font-body)', color: 'var(--color-neutral-700)', margin: '6px 0 4px' }}>{fmtCount(steam.current)} playing right now</div>
            )}
          </div>
        )}
            ...
            <div className="no-scrollbar" style={{ display: 'flex', gap: 10, marginTop: 9, overflowX: 'auto', paddingBottom: 4 }}>
              {similar.map((s) => (
                <div key={s.id} style={{ flex: 'none', width: 104, cursor: 'pointer' }} onClick={(e) => onOpenGame(s, e)}>
                  <div style={{ width: 104, height: 139, borderRadius: 'var(--radius-md)', overflow: 'hidden' }}>
                    <CoverArt game={s} detail={getDetail(s.id)} />
                  </div>
...
export default function App() {
  ...
  useEffect(() => {
    fetch('/api/sheet')
      ...
      .then((csv) => {
        const list = parseSheet(csv).map((g, i) => ({ ...g, i }));
        setGames(list);
        setSyncedAt(Date.now());
        list.forEach((g) => requestDetail(g));
      })
  ...
  const filtered = useMemo(() => {
    ...
    if (chip === 'Meta 90+') list = list.filter((g) => (getDetail(g.id)?.metacritic ?? 0) >= 90);
    if (chip === 'IGN 9.5+') list = list.filter((g) => (ignFor(g.title) ?? 0) >= 9.5);
    if (chip === 'Newest') {
      list.sort((a, b) => (Number(getDetail(b.id)?.year) || 0) - (Number(getDetail(a.id)?.year) || 0));
    }
    return list;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [games, status, chip, open]);

  const openGame = (g, e) => {
    const card = e.currentTarget.querySelector('[data-cover]');
    const rect = card ? card.getBoundingClientRect() : null;
    requestDetail(g, true);
    setOpen({ game: g, rect });
  };

  const syncedLabel = syncedAt
    ? (() => { const m = Math.round((Date.now() - syncedAt) / 60000); return m < 1 ? 'synced just now' : `synced ${m}m ago`; })()
    : 'syncing…';
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: 'calc(env(safe-area-inset-top, 0px) + 18px) 14px 8px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 10 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 3, paddingBottom: 4 }}>
            ...
            <div style={{ font: '400 10px var(--font-body)', color: 'var(--color-neutral-600)' }}>{syncedLabel}</div>
          </div>
        </div>
        ...
        <div className="no-scrollbar" style={{
          ...
          {loadError && (
            <div style={{ gridColumn: '1 / -1', padding: 20, font: '600 13px var(--font-body)', color: 'var(--color-neutral-700)' }}>
              Couldn't reach the sheet — pull to refresh or try again later. ({loadError})
            </div>
          )}
          {filtered.map((g) => {
            const detail = getDetail(g.id);
            return (
      ...
      {open && (
        <DetailSheet
          ...
          onOpenGame={(g, e) => {
            ...
            setOpen({ game: g, rect: el ? el.getBoundingClientRect() : null });
          }}
        />
      )}
    </div>
